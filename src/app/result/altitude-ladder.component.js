 ...
 */
let AltitudeLadderComponent = class AltitudeLadderComponent {
    ...
    flightY = computed(() => this.y(this.heightM()));
    ariaLabel = computed(() => this.matches().length === 0
        ? this.i18n.t('ladder.free')
        : this.matches()
            .map((m) => `${m.zone.name}: ${m.zone.altitude.lower}–${m.zone.altitude.upper} m`)
            .join('; '));
};
AltitudeLadderComponent = __decorate([
    Component({
...
], AltitudeLadderComponent);
export { AltitudeLadderComponent };
const FILL = {
