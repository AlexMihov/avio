...
const rawJson = await (async () => {
    const zip = await JSZip.loadAsync(readFileSync(new URL('./__fixtures__/bgr_zones_30072026.zip', import.meta.url)));
...
describe('bulgaria normalize', () => {
    ...
    it('maps restrictions without loss', () => {
        const counts = zones.reduce((a, z) => ((a[z.restriction] = (a[z.restriction] ?? 0) + 1), a), {});
    ...
    it('extracts the notice period from the message when stated', () => {
        const fiveWorkingDays = zones.find((z) => (z.text.source ?? '').includes('5 работни дни'));
