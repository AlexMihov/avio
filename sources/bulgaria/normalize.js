 ...
 */
function noticeDaysFrom(message) {
    if (!message)
        return undefined;
    if (/месец/i.test(message))
        return 30;
...
export function normalize(raw) {
    ...
    for (const f of doc.features) {
        ...
        if (g.upperVerticalReference !== g.lowerVerticalReference) {
            warnings.push(`${id}: mixed vertical references (${g.lowerVerticalReference}/${g.upperVerticalReference})`);
        }
        const reference = g.upperVerticalReference === 'AMSL' ? 'AMSL' : 'AGL';
        const app = f.applicability?.[0] ?? { permanent: 'YES' };
        const applicability = String(app.permanent).toUpperCase() === 'YES'
            ? { permanent: true }
            : { permanent: false, start: app.startDateTime, end: app.endDateTime };
        const auth = f.zoneAuthority?.[0] ?? {};
        // Trailing whitespace varies between otherwise identical messages upstream.
        const source = typeof f.message === 'string' ? f.message.trim() : null;
        ...
        zones.push({
            ...
            name: f.name ?? id,
            restriction,
            reasons: (f.reason ?? []).map((r) => String(r).toUpperCase()),
            altitude: {
            ...
            conditions: (f.restrictionConditions ?? []).map((c) => c.trim()),
        });
    }
